...
const BLOCK_TYPES = [
  ...
  { type: 'heading3', label: 'Heading 3', icon: 'H3', desc: 'Small section heading' },
  { type: 'paragraph', label: 'Paragraph', icon: '¶', desc: 'Plain text block' },
  { type: 'quote', label: 'Quote', icon: '"', desc: 'Block quotation' },
  { type: 'code', label: 'Code Block', icon: '<>', desc: 'Code with syntax highlighting' },
  ...
  { type: 'bulletList', label: 'Bullet List', icon: '•', desc: 'Unordered list' },
  { type: 'numberedList', label: 'Numbered List', icon: '1.', desc: 'Ordered list' },
  { type: 'checklist', label: 'Checklist', icon: '☑', desc: 'Task checklist' },
  { type: 'columns', label: 'Two Columns', icon: '⊟', desc: 'Two-column layout' },
];
...
export default function Editor({ page, onUpdatePage, allPages, onSelectPage, isLocked }) {
  ...
  useEffect(() => { onUpdatePage(page.id, { blocks }); }, [blocks]); // eslint-disable-line
  ...
  const insertBlocksAfter = useCallback((afterId, newBlocks) => {
    const idx = blocks.findIndex(b => b.id === afterId);
    setBlocks(prev => {
      const next = [...prev];
  ...
  const openBlockMenu = useCallback((blockId, rect) => {
    ...
    setBlockMenu({ top: rect.bottom + window.scrollY + 4, left: rect.left, afterId: blockId });
  }, []);

  const handleMenuSelect = useCallback((type) => {
    if (blockMenu) { const newId = addBlockAfter(blockMenu.afterId, type); setActiveBlockId(newId); }
    setBlockMenu(null);
  }, [blockMenu, addBlockAfter]);

  // Smart paste handler
  const handleSmartPaste = useCallback((blockId, text) => {
    const detectedBlocks = detectAndConvertPaste(text);
    if (detectedBlocks.length === 1 && detectedBlocks[0].type === 'paragraph') {
      ...
      updateBlock(blockId, { type: first.type, content: first.content || '', ...(first.tableData ? { tableData: first.tableData } : {}), ...(first.items ? { items: first.items } : {}), ...(first.language ? { language: first.language } : {}) });
...
<body>
  ...
  <div class="page-meta">Last updated ${new Date(page.updatedAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</div>
    ...
    <div className="editor" ref={editorRef}>
      <div className="editor-inner">
        ...
        <div className="page-header">
          <div className="page-header-top">
            <div className="page-header-icon">
              <span className="big-icon"><PageIconDisplay icon={page.icon} size={44} /></span>
            </div>
            <button
          ...
          {editingTitle ? (
            ...
            <h1 className="page-title-display" onClick={() => !isLocked && setEditingTitle(true)}>
              {page.title || 'Untitled'}
            </h1>
          )}
          <div className="page-meta">
            Last updated {new Date(page.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
          </div>
        </div>
        ...
        {subPages.length > 0 && (
          <div className="subpages-section">
            ...
            <div className="subpages-grid">
              {subPages.map(sp => (
                <button key={sp.id} className="subpage-card" onClick={() => onSelectPage(sp.id)}>
                  <span className="subpage-card-icon"><PageIconDisplay icon={sp.icon} size={18} /></span>
        ...
        <div className="page-footer">
          <span><strong>Akhil Antony Joseph Rio</strong></span>
        </div>
      </div>
...
function renderBlockForPrint(block) {
  ...
  switch (block.type) {
    case 'heading1': return `<div class="block h1-text" style="${align}">${esc(block.content)}</div>`;
    case 'heading2': return `<div class="block h2-text" style="${align}">${esc(block.content)}</div>`;
    case 'heading3': return `<div class="block h3-text" style="${align}">${esc(block.content)}</div>`;
    case 'paragraph': return `<div class="block p-text" style="${align}">${esc(block.content)}</div>`;
    case 'quote': return `<div class="block quote-text" style="${align}">${esc(block.content)}</div>`;
    case 'divider': return `<hr class="divider"/>`;
    case 'code': return `<div class="block"><div style="font-size:10px;color:#6b6880;margin-bottom:4px">${esc(block.language||'code')}</div><div class="code-block">${esc(block.content)}</div></div>`;
    case 'callout': return `<div class="block callout">${esc(block.content)}</div>`;
    case 'bulletList': return `<div class="block">${(block.items||[]).map(i=>`<div class="list-item"><span class="list-bullet">•</span><span>${esc(i.text)}</span></div>`).join('')}</div>`;
    case 'numberedList': return `<div class="block">${(block.items||[]).map((i,idx)=>`<div class="list-item"><span class="list-bullet">${idx+1}.</span><span>${esc(i.text)}</span></div>`).join('')}</div>`;
    case 'checklist': return `<div class="block">${(block.items||[]).map(i=>`<div class="check-item ${i.checked?'checked':''}"><span>${i.checked?'☑':'☐'}</span><span>${esc(i.text)}</span></div>`).join('')}</div>`;
    case 'table': {
      const td = block.tableData || { headers: [], rows: [] };
      return `<div class="block"><table class="wiki-table"><thead><tr>${td.headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${td.rows.map(r=>`<tr>${r.map(c=>`<td>${esc(c)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
    }
    case 'image': return block.content ? `<div class="block"><img src="${block.content}" style="max-width:100%;border-radius:8px;" alt="${esc(block.caption||'')}"/>${block.caption?`<div style="text-align:center;font-size:12px;color:#6b6880;margin-top:4px">${esc(block.caption)}</div>`:''}</div>` : '';
    default: return '';
  }
}
...
function createEmptyBlock(type) {
  ...
  switch (type) {
    case 'code': return { ...base, language: 'javascript' };
    case 'image': return { ...base, content: '', caption: '' };
    case 'video': return { ...base, content: '', caption: '' };
    case 'callout': return { ...base, content: '💡 ', variant: 'info' };
    case 'table': return { ...base, tableData: { headers: ['Column 1', 'Column 2', 'Column 3'], rows: [['', '', ''], ['', '', '']] } };
    case 'toggle': return { ...base, content: 'Toggle title', children: [{ id: uuidv4(), type: 'paragraph', content: '' }], open: false };
    case 'columns': return { ...base, columns: [{ id: uuidv4(), blocks: [{ id: uuidv4(), type: 'paragraph', content: '' }] }, { id: uuidv4(), blocks: [{ id: uuidv4(), type: 'paragraph', content: '' }] }] };
    case 'checklist': return { ...base, items: [{ id: uuidv4(), text: '', checked: false }] };
    case 'bulletList': return { ...base, items: [{ id: uuidv4(), text: '' }] };
    case 'numberedList': return { ...base, items: [{ id: uuidv4(), text: '' }] };
    default: return base;
  }
}
